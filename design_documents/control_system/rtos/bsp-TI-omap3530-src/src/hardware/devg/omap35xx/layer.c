...
int omap_layer_set_surface(disp_adapter_t * adapter, int dispno, int layer_idx,
	...
	*/
	...
	case DISP_LAYER_FORMAT_YUY2:
		...
		o_ctx->layer[layer_idx].omap_layer_format = (0x0A<<1) ;
		break;
	case DISP_LAYER_FORMAT_UYVY:
		o_ctx->layer[layer_idx].bpp  = 2;
		o_ctx->layer[layer_idx].ppmp = 2;
		o_ctx->layer[layer_idx].omap_layer_format = (0x0B<<1)  ;
		break;
	case DISP_LAYER_FORMAT_YVYU:
		o_ctx->layer[layer_idx].bpp  = 2;
		o_ctx->layer[layer_idx].ppmp = 2;
		o_ctx->layer[layer_idx].omap_layer_format = (0x0B<<1) | (1<<17);
		break;
	case DISP_LAYER_FORMAT_V422:
		o_ctx->layer[layer_idx].bpp  = 2;
		o_ctx->layer[layer_idx].ppmp = 2;
		o_ctx->layer[layer_idx].omap_layer_format = (0x0A<<1) | (1<<17);
		break;
...
int omap_layer_set_chromakey(disp_adapter_t *adapter, int dispno,
	...
	if (chroma_mode & DISP_CHROMA_OP_SRC_MATCH || chroma_mode
		...
		o_ctx->layer[layer_idx].config |= ((0<<11)|(1<<10));
	}
...
int omap_layer_program(disp_adapter_t *adapter, omap_ctx_t *o_ctx, int layer_idx) {
	...
	if (!(o_ctx->layer[layer_idx].ppmp == 0 || o_ctx->layer[layer_idx].ppmp == 0 || o_ctx->layer[layer_idx].bpp == 0)  ) {
	 ...
	 */
	*OMAP_DISPC_CONFIG |= o_ctx->layer[layer_idx].config;
	...
	case OMAP_LAYER_VID1:
		...
		*OMAP_DISPC_VID1_ATTRIBUTES = o_ctx->layer[layer_idx].attribute
				| o_ctx->layer[layer_idx].omap_layer_format | (1<<10) |(1<<9) | (o_ctx->ccfull<<11) | ((o_ctx->vid1_burstsz & OMAP_BURST_MASK)<<14);
	...
	case OMAP_LAYER_VID2:
		...
		*OMAP_DISPC_VID2_ATTRIBUTES = o_ctx->layer[layer_idx].attribute
				| o_ctx->layer[layer_idx].omap_layer_format | (1<<10) | (1<<9) | (o_ctx->ccfull<<11) | ((o_ctx->vid2_burstsz & OMAP_BURST_MASK)<<14);
...
__SRCVERSION( "$URL: http://svn/product/tags/public/bsp/nto641/ti-omap3530-beagle/1.0.0/src/hardware/devg/omap35xx/layer.c $ $Rev: 249398 $" );
